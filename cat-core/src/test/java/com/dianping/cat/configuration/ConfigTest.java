...
public class ConfigTest {
	...
	public void testServer() throws Exception {
		...
		String xml = Files.forIO().readFrom(in, "utf-8");
		ServerConfig config = com.dianping.cat.configuration.server.transform.DefaultSaxParser.parse(xml);

		Assert.assertEquals(true, config.isLocalMode());
		Assert.assertNotNull(config.getStorage());
		Assert.assertNotNull(config.getConsumer());
		Assert.assertNotNull(config.getConsole());
	}
}
