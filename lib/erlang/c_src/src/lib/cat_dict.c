...
dict *catDictCreate(dictType *type,
                    void *privDataPtr) {
...
int catDictResize(dict *ht) {
    ...
    return catDictExpand(ht, minimal);
}
...
int catDictReplace(dict *ht, void *key, void *val) {
    ...
    if (catDictAdd(ht, key, val) == DICT_OK)
        return DICT_OK;
    /* It already exists, get the entry */
    entry = catDictFind(ht, key);
...
static int _dictExpandIfNeeded(dict *ht) {
    ...
    if (ht->size == 0)
        return catDictExpand(ht, DICT_HT_INITIAL_SIZE);
    if (ht->used == ht->size)
        return catDictExpand(ht, ht->size * 2);
    return DICT_OK;
}
...
unsigned int _dictStringCopyHTHashFunction(const void *key) {
    return catDictGenHashFunction(key, strlen(key));
}
...
void _dictValFreeDestructor(void *privdata, void *val) {
    DICT_NOTUSED(privdata);
...
dictType dictTypeHeapStringCopyKeyFreeVal = {
        _dictStringCopyHTHashFunction,        /* hash function */
        _dictStringCopyHTKeyDup,              /* key dup */
        NULL,                               /* val dup */
        _dictStringCopyHTKeyCompare,          /* key compare */
        _dictStringCopyHTKeyDestructor,       /* key destructor */
        _dictValFreeDestructor  /* val destructor */
};
