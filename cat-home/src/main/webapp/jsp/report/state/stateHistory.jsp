...
<table width="80%">
	...
	<tr class='even'>
		...
		<td>丢失消息总量</td>
		<c:choose>
			<c:when test="${model.state.total.totalLoss > 0}">
						<td class="right" style="color: red;">${w:format(model.state.total.totalLoss,'#,###,###,###,##0.#')}</td>
					</c:when>
			<c:otherwise>
	...
	<tr class='even'>
		...
		<c:choose>
			<c:when test="${model.state.total.dumpLoss > 0}">
						<td class="right" style="color: red;">${w:format(model.state.total.dumpLoss,'#,###,###,###,##0.#')}</td>
					</c:when>
			<c:otherwise>
...
<table width="100%">
	<tr class='odd'>
		<td width="15%"><a
					href="?op=history&domain=${model.domain}&ip=${model.ipAddress}&date=${model.date}&sort=domain">处理项目列表</a></td>
		<td width="5%"><a
					href="?op=history&domain=${model.domain}&ip=${model.ipAddress}&date=${model.date}&sort=total">处理消息总量</a></td>
		<td width="5%"><a
