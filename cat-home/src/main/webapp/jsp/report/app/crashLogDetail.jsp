...
<c:set var="report" value="${model.problemReport}" />
	<div class="report">
		...
		<c:choose>
				...
				<td><span class="text-danger title">【报表时间】</span><span class="text-success">${w:format(report.startTime,'yyyy-MM-dd HH:mm')} to ${w:format(report.endTime,'yyyy-MM-dd HH:mm')}</span>
					</td><td align="right"><span class="text-danger switch"><a class="switch" href="${model.baseUri}?op=historyCrashLog&query1=AndroidCrashLog;;;;"><span class="text-danger">【切到历史】</span></a></span>
		...
		<c:otherwise>
			<c:if test="${payload.action.name eq 'historyCrashLog'}">
				<td><span class="text-danger title">【报表时间】</span><span class="text-success">${w:format(payload.historyStartDate,'yyyy-MM-dd HH:mm')} to ${w:format(payload.historyDisplayEndDate,'yyyy-MM-dd HH:mm')}</span>
				</td><td align="right"><span class="text-danger switch"><a class="switch" href="${model.baseUri}?op=crashLog&query1=AndroidCrashLog;;;;"><span class="text-danger">【切到小时】</span></a></span>
