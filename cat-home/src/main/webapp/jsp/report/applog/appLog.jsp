...
<a:mobile>
	...
	<div class="report">
		<table class="table ">
			<tr>
				<td colspan="2">
					...
					<div class="input-group" style="float:left;width:30px">
						...
						<input type="text" id="unionId" style="width: 120px; height:34px"/>
					</div>
				...
				<td>
					<div class="nav-search nav" id="nav-search" style="margin-top: 12px">
						&nbsp;[ <a
						href="${model.baseUri}?op=appLog&appLogQuery.day=${payload.appLogQuery.day}&appLogQuery.appName=${payload.appLogQuery.appName}&appLogQuery.unionId=${payload.appLogQuery.unionId}&appLogQuery.query=${payload.appLogQuery.query}&appLogQuery.step=-1&appLogQuery.platform=${payload.appLogQuery.platform}">-1d</a>
						]&nbsp;
						&nbsp;[ <a
						href="${model.baseUri}?op=appLog&appLogQuery.day=${payload.appLogQuery.day}&appLogQuery.appName=${payload.appLogQuery.appName}&appLogQuery.unionId=${payload.appLogQuery.unionId}&appLogQuery.query=${payload.appLogQuery.query}&appLogQuery.step=1&appLogQuery.platform=${payload.appLogQuery.platform}">+1d</a>
						]&nbsp;
						&nbsp;[ <a
						href="${model.baseUri}?op=appLog&appLogQuery.appName=${payload.appLogQuery.appName}&appLogQuery.unionId=${payload.appLogQuery.unionId}&appLogQuery.query=${payload.appLogQuery.query}&appLogQuery.platform=${payload.appLogQuery.platform}">now</a>
						]&nbsp;
					</div>
				</td>
			</tr>
			<tr>
				<td width="100px;" style="vertical-align: middle">&nbsp;&nbsp;APP版本</td>
				<td>
					<div style="margin-left: -15px">
						<label class="btn btn-success btn-sm">
							<input type="checkbox" id="appVersionAll"
								   onclick="clickAll('${model.appLogDisplayInfo.fieldsInfo.appVersions}', 'appVersion')"
								   unchecked>All
						</label>
						<c:forEach var="item" items="${model.appLogDisplayInfo.fieldsInfo.appVersions}"
								   varStatus="status">
							<label class="btn btn-info btn-sm">
								<input type="checkbox"
									   id="appVersion_${item}"
									   value="${item}"
									   onclick="clickMe('${model.appLogDisplayInfo.fieldsInfo.appVersions}', 'appVersion')"
									   unchecked>${item}
							</label>
						</c:forEach>
					</div>
			...
			<tr>
				<td width="60px;" style="vertical-align: middle">&nbsp;&nbsp;平台版本</td>
				<td>
					<div style="margin-left: -15px">
						<label class="btn btn-success btn-sm">
							<input type="checkbox" id="platformVersionAll"
								   onclick="clickAll('${model.appLogDisplayInfo.fieldsInfo.platVersions}', 'platformVersion')"
								   unchecked>All
						</label>
						<c:forEach var="item" items="${model.appLogDisplayInfo.fieldsInfo.platVersions}"
								   varStatus="status">
							<label class="btn btn-info btn-sm">
								<input type="checkbox"
									   id="platformVersion_${item}"
									   value="${item}"
									   onclick="clickMe('${model.appLogDisplayInfo.fieldsInfo.platVersions}', 'platformVersion')"
									   unchecked>${item}
							</label>
						</c:forEach>
					</div>
			...
			<tr>
				<td width="60px;" style="vertical-align: middle">&nbsp;&nbsp; 级别</td>
				<td>
					<div style="margin-left: -15px">
						<label class="btn btn-success btn-sm">
							<input type="checkbox" id="levelAll"
								   onclick="clickAll('${model.appLogDisplayInfo.fieldsInfo.levels}', 'level')"
								   unchecked>All
						</label>
						<c:forEach var="item" items="${model.appLogDisplayInfo.fieldsInfo.levels}"
								   varStatus="status">
							<label class="btn btn-info btn-sm">
								<input type="checkbox" id="level_${item}"
									   value="${item}"
									   onclick="clickMe('${model.appLogDisplayInfo.fieldsInfo.levels}', 'level')"
									   unchecked>${item}
							</label>
						</c:forEach>
					</div>
			...
			<tr>
				<td width="60px;" style="vertical-align: middle">&nbsp;&nbsp;设备</td>
				<td>
					<select multiple="true" class="chosen-select tag-input-style" id="device_select" name="devices"
							data-placeholder="Choose devices..." style="margin-left: -20px">
			...
			<div class="tab-pane active" id="report">
				<table class="table table-hover table-striped table-condensed">
					...
					<c:forEach var="error" items="${model.appLogDisplayInfo.msgs}" varStatus="typeIndex">
						<tr>
							<td>
								<a href="" class="app_graph_link" data-status="${typeIndex.index}" msg="${error.msg}">[::
									show ::]</a>
									${error.msg}</td>
