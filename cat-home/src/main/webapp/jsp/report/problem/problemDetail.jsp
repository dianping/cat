	...
	<c:forEach var="statistics" items="${model.detailStatistics.status}" varStatus="typeIndex">
		<tr>
			...
			<c:forEach var="status" items="${statistics.value.status}"
				...
				<td >
					<c:forEach var="links" items="${status.value.links}" varStatus="linkIndex"><a title="${linkIndex.index }" href="/cat/r/m/${links}?domain=${model.domain}">${linkIndex.first?'L':(linkIndex.last?'g':'o')}</a></c:forEach>
				</td>
				<c:if test="${index.index != 0}">
				</tr>
				</c:if>
