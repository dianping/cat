	...
	<c:forEach var="statistics" items="${model.allStatistics.status}"
		...
		<tr>
			<td rowspan="${w:size(statistics.value.status)*2}"
				...
				<a href="?op=historyGroupGraph&domain=${model.domain}&date=${model.date}&ip=${model.ipAddress}&reportType=${model.reportType}&type=${statistics.value.type}${model.customDate}" class="history_graph_link" data-status="${typeIndex.index}">[:: show ::]</a>
			...
			<c:forEach var="status" items="${statistics.value.status}"
				...
				<td class="${index.index mod 2 != 0 ? 'even' : 'odd'}">
					<a href="?op=historyGroupGraph&domain=${model.domain}&date=${model.date}&ip=${model.ipAddress}&reportType=${model.reportType}&type=${statistics.value.type}&status=${status.value.status}${model.customDate}" class="problem_status_graph_link" data-status="${statistics.value.type}${status.value.status}">[:: show ::]</a>
					&nbsp;${status.value.status}
				</td>
