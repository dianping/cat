...
<br/>
<h4 class="text-success">1. 响应时间阈值<a href="/cat/s/config?op=transactionRule">配置</a></h4>
<table style="width:90%" class="table table-striped table-bordered table-condensed">
	...
	<tr>
		<td>异常名称</td>
		<td>异常名称，<span class="text-error">必需，</span>当设置为“Total”时，是针对当前项目组所有异常总数阈值进行设置；当设置为特定异常名称时，针对当前项目组所有同名的异常阈值进行设定</td>
	</tr>
	<tr>
		<td>Warning阈值</td>
...
<table style="width:90%" class="table table-striped table-bordered table-condensed">
	...
	<tr>
		<td>异常名称</td>
		<td>异常名称，<span class="text-error">必需，</span>当设置为特定异常名称时，过滤当前项目组所有同名的异常</td>
	</tr>
</table>
<br/>
<h4 class="text-success">3. 异常告警<a href="/cat/s/config?op=projects">组邮件配置</a></h4>
<p>针对每个项目组，可以进行组邮件和联系人的配置，CAT将根据此配置进行邮件的发送。</p>
<p class="text-error">注意：不设置组邮件的话，您将错过第一时间知晓异常的机会！</p>
<br/>
