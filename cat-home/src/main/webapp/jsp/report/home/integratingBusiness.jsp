...
<table style="width:90%" class="table table-striped table-bordered table-condensed">
	...
	<tr><td>timestamp</td><td>时间戳<span class="text-error">必需，仅仅为数字。如果确实，选取服务器当前时间</span></td></tr>
...
<pre>
	http://cat.dianpingoa.com/cat/r/monitor?group=myGroup&domain=myApp&key=myKey&timestamp=1404815988&op=count
</pre>
<p> 2).op = avg时，用于记录一个指标出现的平均值</p>
<pre>
	http://cat.dianpingoa.com/cat/r/monitor?group=myGroup&domain=myApp&key=myKey&timestamp=1404815988&op=avg&avg=500
</pre>
<p> 3).op = sum时，用于记录一个指标出现的总和</p>
<pre>
	http://cat.dianpingoa.com/cat/r/monitor?group=myGroup&domain=myApp&key=myKey&timestamp=1404815988&op=sum&sum=500
</pre>
<p>返回说明</p>
<pre>
