...
<table class="table table-hover table-striped table-condensed table-bordered"  style="width:100%">
	...
	<tr>
		<c:forEach var="item" items="${model.currentOperations}">
			<th class="right"><a data-rel="tooltip" data-placement="left" title="一段时间内操作总量" href="?op=${payload.action.name}&type=${payload.type}&domain=${model.domain}&id=${payload.id}&ip=${model.ipAddress}&date=${model.date}&operations=${payload.operations}&sort=${item};count">总量</a></th>
			<c:if test="${payload.type eq 'SQL'}">
				<th class="right"><a data-rel="tooltip" data-placement="left" title="一段时间内长时间(超过1s)操作总量" href="?op=${payload.action.name}&type=${payload.type}&domain=${model.domain}&id=${payload.id}&ip=${model.ipAddress}&date=${model.date}&operations=${payload.operations}&sort=${item};long">慢查询次数</a></th>
			</c:if>
			<c:if test="${payload.type eq 'Cache'}">
				<th class="right"><a data-rel="tooltip" data-placement="left" title="一段时间内长时间(超过50ms)操作总量" href="?op=${payload.action.name}&type=${payload.type}&domain=${model.domain}&id=${payload.id}&ip=${model.ipAddress}&date=${model.date}&operations=${payload.operations}&sort=${item};long">慢查询次数</a></th>
			</c:if>
			<c:if test="${payload.type eq 'RPC'}">
				<th class="right"><a data-rel="tooltip" data-placement="left" title="一段时间内长时间(超过100ms)操作总量" href="?op=${payload.action.name}&type=${payload.type}&domain=${model.domain}&id=${payload.id}&ip=${model.ipAddress}&date=${model.date}&operations=${payload.operations}&sort=${item};long">慢调用次数</a></th>
			</c:if>
			<th class="right"><a data-rel="tooltip" data-placement="left" title="一段时间内操作平均响应时间(ms)" href="?op=${payload.action.name}&type=${payload.type}&domain=${model.domain}&id=${payload.id}&ip=${model.ipAddress}&date=${model.date}&operations=${payload.operations}&sort=${item};avg">平均耗时</a></th>
			<th class="right"><a data-rel="tooltip" data-placement="left" title="一段时间内错误操作总数" href="?op=${payload.action.name}&type=${payload.type}&domain=${model.domain}&id=${payload.id}&ip=${model.ipAddress}&date=${model.date}&operations=${payload.operations}&sort=${item};error">错误总数</a></th>
		</c:forEach>
	</tr>
