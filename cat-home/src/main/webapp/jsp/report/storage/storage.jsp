...
<table class="table table-hover table-striped table-condensed table-bordered"  style="width:100%">
	...
	<tr>
		<c:forEach var="item" items="${model.currentOperations}">
			<th class="right"><a href="?op=${payload.action.name}&type=${payload.type}&domain=${model.domain}&id=${payload.id}&ip=${model.ipAddress}&date=${model.date}&operations=${payload.operations}&sort=${item};count">Count</a>
 			<i class="glyphicon glyphicon-question-sign" data-rel="tooltip" data-placement="left" title="一分钟内操作总量"></i></th>
			<th class="right"><a href="?op=${payload.action.name}&type=${payload.type}&domain=${model.domain}&id=${payload.id}&ip=${model.ipAddress}&date=${model.date}&operations=${payload.operations}&sort=${item};long">Long</a>
			<i class="glyphicon glyphicon-question-sign" data-rel="tooltip" data-placement="left" title="一分钟内长时间(超过一秒)操作总量"></i></th>
			<th class="right"><a href="?op=${payload.action.name}&type=${payload.type}&domain=${model.domain}&id=${payload.id}&ip=${model.ipAddress}&date=${model.date}&operations=${payload.operations}&sort=${item};avg">Avg</a>
			<i class="glyphicon glyphicon-question-sign" data-rel="tooltip" data-placement="left" title="一分钟内操作平均响应时间"></i></th>
			<th class="right"><a href="?op=${payload.action.name}&type=${payload.type}&domain=${model.domain}&id=${payload.id}&ip=${model.ipAddress}&date=${model.date}&operations=${payload.operations}&sort=${item};error">Error</a>
