				...
				<c:otherwise>
   	  		...
   	  		<c:choose>
					<c:when test="${model.database eq database}">
						<a href="?op=history&domain=${model.domain}&database=${database}&date=${model.date}"
									class="current">${database}</a>
					</c:when>
					<c:otherwise>
						<a href="?op=history&domain=${model.domain}&database=${database}&date=${model.date}">${database}</a>
					</c:otherwise>
				</c:choose>
   	 		&nbsp;]&nbsp;
			 </c:forEach>
...
<table class="sql">
			<tr>
			    <th></th>
				<th class="left"><a
					href="?op=history&database=${model.database}&date=${model.date}&domain=${model.domain}&sort=name">Table</a></th>
