...
<a:config>
    ...
    <form name="addSubmit" id="form" method="post" action="">
			<table class="table table-striped table-condensed  ">
				<tr>
					...
					<td width="35%" >
						<c:if test="${not empty model.businessItemConfig.id}">
                            <input name="businessItemConfig.id" id="businessItemConfig_id"
                                   value="${model.businessItemConfig.id}" readonly required/>
						</c:if>
						<c:if test="${empty  model.businessItemConfig.id}">
                            <input name="businessItemConfig.id" id="businessItemConfig_id"
                                   value="${model.businessItemConfig.id}" required/>
						</c:if>
					</td>
				</tr>
				<tr>
					<td  style="text-align:right" class="text-success">显示标题</td>
                    <td><input name="businessItemConfig.title" id="businessItemConfig_title"
                               value="${model.businessItemConfig.title}" required/></td>
					<td  style="text-align:right" class="text-success">显示顺序（数字）</td>
                    <td><input name="businessItemConfig.viewOrder" id="businessItemConfig_viewOrder"
                               value="${model.businessItemConfig.viewOrder}" required/></td>
				</tr>
				<tr>
					...
					<td>
						<c:choose>
							<c:when test="${model.businessItemConfig.showAvg}">
                                <input type="radio" name="businessItemConfig_showAvg" value="true"
                                       checked/>是 &nbsp;&nbsp;&nbsp;
					...
					<td>
						<c:choose>
							<c:when test="${model.businessItemConfig.showSum}">
                                <input type="radio" name="businessItemConfig_showSum" value="true"
                                       checked/>是 &nbsp;&nbsp;&nbsp;
				...
				<tr>
                    <td style="text-align:center" colspan='4'><input class='btn btn-primary btn-xs'
                                                                     id="addOrUpdateNodeSubmit" type="text"
