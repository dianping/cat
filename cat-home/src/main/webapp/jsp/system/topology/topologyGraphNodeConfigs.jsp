					    ...
					    <h4 class="text-center text-error">拓扑图节点配置信息：${item.key}</h4>
				     	<table class="table table-striped table-bordered table-condensed table-hover">
				     		...
				     		<tr class="text-error"><td><h5>默认值</h5></td><td><h5 class="text-right">${value.defaultWarningThreshold}</h5></td>
						     	...
						     	<td style="text-align:center;vertical-align:middle;"><a href="?op=topologyGraphNodeConfigAdd&type=${item.key}&domain=ALL" class="btn update btn-primary btn-sm">修改</a></td>
					     		</tr>
					     	<c:forEach var="domainConfig" items="${value.domainConfigs}">
					     		...
					     		<td style="text-align:center"><a href="?op=topologyGraphNodeConfigAdd&type=${item.key}&domain=${temp.id}" class="btn update btn-primary btn-sm">修改</a>
						     	<a href="?op=topologyGraphNodeConfigDelete&type=${item.key}&domain=${temp.id}" class="btn btn-primary btn-sm btn-danger delete">删除</a></td>
					     		</tr>
					     	</c:forEach>
				     	</table>
				     </div>
