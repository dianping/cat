	...
	<div class="body-content">
		...
		<div class="content-right">
			...
			<thead>
				<tr class="odd">
					<td>项目名</td>
					<td>操作&nbsp;&nbsp;  <a href="?op=alarmRuleAdd&type=exception" target="_blank">新增</a></td>
				</tr></thead><tbody>
					...
					<tr>
						...
						<td><c:choose>
								<c:when test="${item.subscriberState == 0}">
									<a href="?op=exceptionAlarmRuleSub&alarmRuleId=${item.alarmRule.id}&subState=0" onclick="return sub(this)">订阅</a>
								</c:when>
								<c:otherwise>
									<a href="?op=exceptionAlarmRuleSub&alarmRuleId=${item.alarmRule.id}&subState=1" onclick="return sub(this)">取消</a>
								</c:otherwise>
							</c:choose> 
							<a href="?op=alarmRuleUpdate&alarmRuleId=${item.alarmRule.id}"  target="_blank">编辑</a> 
							<a href="?op=exceptionAlarmRuleDelete&alarmRuleId=${item.alarmRule.id}">删除</a> 
						</td>
					</tr>
				</c:forEach></tbody>
			</table>
