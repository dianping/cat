					...
					<tr>
						...
						<td><c:choose>
								<c:when test="${item.subscriberState == 0}">
									<a href="?op=exceptionAlarmRuleSub&alarmRuleId=${item.alarmRule.id}&subState=0">订阅</a>
								</c:when>
								<c:otherwise>
									<a href="?op=exceptionAlarmRuleSub&alarmRuleId=${item.alarmRule.id}&subState=1">取消</a>
								</c:otherwise>
							</c:choose> 
