			...
			<table class="table table-striped table-bordered table-condensed table-hover">
		     	...
		     	<c:forEach var="item" items="${model.ruleItems}" varStatus="status">
		     		...
		     		<td style="text-align:center;white-space: nowrap">
		     			<a href="?op=appRuleUpdate&key=${item.id}" class="btn update btn-primary btn-small">修改规则</a>
			     		<a href="?op=appRulDelete&key=${item.id}" class="btn btn-primary btn-small btn-danger delete">删除</a>
			     	</td>
		     		</tr>
		     	</c:forEach>
	     	</table>
		    <a href="?op=appRuleUpdate&key=${item.id}" class="btn update btn-primary btn-small btn-success">添加监控规则<i class="icon-plus icon-white"></i></a>
		</div>
	</div>
</a:body>