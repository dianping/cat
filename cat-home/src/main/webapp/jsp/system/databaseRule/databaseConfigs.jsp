		...
		$(document).ready(function() {
			...
			<table class="table table-striped table-condensed table-bordered  table-hover">
	     		<thead><tr>
	     			...
	     			<th width="7%">平均值</th>
	     			<th width="4%">总和</th>
	     			<th width="8%">操作 <a href="?op=databaseRuleUpdate&key=${item.id}" class="btn btn-primary btn-xs" >
		     	...
		     	<c:forEach var="item" items="${model.ruleItems}" varStatus="status">
					...
					<td><a href="?op=databaseRuleUpdate&key=${item.id}" class="btn btn-primary btn-xs">
						<i class="ace-icon fa fa-pencil-square-o bigger-120"></i></a>
						<a href="?op=databaseRulDelete&key=${item.id}" class="btn btn-danger btn-xs delete" >
