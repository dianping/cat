...
<div class="report">
	<table class="header">
		<tr>
			...
			<td class="nav">
					<span class="text-error">【<a href="?domain=${model.domain}&op=browser" class="switch"><span class="text-error">切到小时模式</span></a>】</span>
					&nbsp;&nbsp;<c:forEach var="nav" items="${model.historyNavs}">
					<c:choose>
						<c:when test="${nav.title eq model.reportType}">
								&nbsp;[ <a href="?op=historyBrowser&domain=${model.domain}&date=${model.date}&reportType=${nav.title}" class="current">${nav.title}</a> ]
						</c:when>
						<c:otherwise>
								&nbsp;[ <a href="?op=historyBrowser&domain=${model.domain}&date=${model.date}&reportType=${nav.title}">${nav.title}</a> ]
						</c:otherwise>
					</c:choose>
				</c:forEach>
				&nbsp;[ <a href="?op=historyBrowser&domain=${model.domain}&date=${model.date}&reportType=${model.reportType}&step=-1&${navUrlPrefix}">${model.currentNav.last}</a> ]
				&nbsp;[ <a href="?op=historyBrowser&domain=${model.domain}&date=${model.date}&reportType=${model.reportType}&step=1&${navUrlPrefix}">${model.currentNav.next}</a> ]
				&nbsp;[ <a href="?op=historyBrowser&domain=${model.domain}&reportType=${model.reportType}&nav=next&${navUrlPrefix}">now</a> ]
			</td>
		</tr>
	</table>
					...
					<tr>
						...
						<c:forEach var="productline" items="${detail.projectLines}" varStatus="index">
								...
								<td><div class="domain"><c:forEach var="domain" items="${productline.value.lineDomains}">&nbsp;<c:choose><c:when test="${model.domain eq domain}"><a
														href="?op=historyBrowser&domain=${domain}&date=${model.date}&reportType=${model.reportType}"
														class="current">[&nbsp;${domain}&nbsp;]</a></c:when>
														<c:otherwise><a
														href="?op=historyBrowser&domain=${domain}&date=${model.date}&reportType=${model.reportType}">[&nbsp;${domain}&nbsp;]</a>
