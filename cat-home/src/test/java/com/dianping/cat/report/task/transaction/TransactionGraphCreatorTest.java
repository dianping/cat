	...
	public void test() throws Exception {
		...
		for (Graph graph : graphs) {
			...
			for (String line : lines) {
				...
				range.sum = getSum(records[SummaryOrder.SUM.ordinal()]);
			...
			for (String line : lines) {
				...
				range.sum = getSum(records[DetailOrder.SUM.ordinal()]);
				result.put(ip + ',' + type + ',' + name, range);
			}
		}

		TransactionHelpGraphCreator creator2 = new TransactionHelpGraphCreator();
		List<Graph> graphs2 = creator2.splitReportToGraphs(date, "MobileApi", "transaction", report);
		...
		for (String str : result.keySet()) {
			...
			Assert.assertEquals(true, range1.sum - range2.sum < 0.1);
		}
	}
