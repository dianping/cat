...
public class TransactionReportMerger extends DefaultMerger {
	...
	public Machine mergesForAllMachine(TransactionReport report) {
		Machine machine = new Machine(ReportConstants.ALL_IP);

		for (Machine m : report.getMachines().values()) {
			if (!m.getIp().equals(ReportConstants.ALL_IP)) {
	...
	public void visitTransactionReport(TransactionReport transactionReport) {
		...
		if (m_allIp) {
			m_allMachines = report.findOrCreateMachine(ReportConstants.ALL_IP);
		}

		if (m_allName) {
			m_allNames = report.findOrCreateMachine(m_ip)
					.findOrCreateType(m_type).findOrCreateName("ALL");
		}
