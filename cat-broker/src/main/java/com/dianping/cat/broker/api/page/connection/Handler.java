...
import com.dianping.cat.broker.api.page.batch.Context;
...
public class Handler implements PageHandler<Context>, LogEnabled {
	...
	@InboundActionMeta(name = "connection")
	public void handleInbound(Context ctx) throws ServletException, IOException {
		// display only, no action here
	}

	@Override
	@OutboundActionMeta(name = "connection")
	public void handleOutbound(Context ctx) throws ServletException, IOException {
		...
		if (userIp != null) {
			...
			success = false;
			Cat.logEvent("UnknownIp", API_TYPE, Event.SUCCESS, null);
	...
	private void offerQueue(ProtoData appData) {
		...
		if (!success) {
			...
			if (m_error % 1000 == 0) {
				Cat.logEvent("Discard", API_TYPE, Event.SUCCESS, null);
