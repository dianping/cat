...
<a:report title="Browser Report" navUrlPrefix="op=browser&domain=${model.domain}" timestamp="${w:format(model.creatTime,'yyyy-MM-dd HH:mm:ss')}">
...
<script type="text/javascript">
	$(document).ready(function() {
		...
		$('.switch')[1].href="?op=historyBrowser&domain=${model.domain}&date=${model.date}";
		for(var i=0;i<$('.domainItem').length;i++){
			$('.domainItem')[i].href=$('.domainItem')[i].href+"&op=browser";
		}
	});
</script>
